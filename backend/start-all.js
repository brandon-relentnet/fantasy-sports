...
const backends = [
  ...
  {
    name: "fantasy-yahoo",
    dir: path.join(__dirname, "fantasy-yahoo"),
    command: "node",
    args: ["dist/server.js"],
    logFile: path.join(logsDir, "fantasy-yahoo.log"),
    port: parseInt(process.env.FANTASY_YAHOO_PORT || "4002", 10),
    readyPattern: /✅ Fantasy Yahoo server running on port \d+/,
    urls: (port) => [
      `http://localhost:${port}/leagues`,
      `http://localhost:${port}/team/<teamKey>/roster`,
      `http://localhost:${port}/health`
    ]
  },
];
