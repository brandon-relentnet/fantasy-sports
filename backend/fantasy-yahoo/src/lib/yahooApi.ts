...
import { decodeYahooStats, getKeyStats } from './yahooStatIds';
...
export class YahooFantasyAPI {
  ...
  async getUserLeagues(gameKey = 'mlb') {
    ...
    const leagues: any[] = [];
    try {
      const users = (data as any)?.fantasy_content?.users?.[0]?.user;
    ...
    } catch (e) { logToFile('Error parsing leagues', String(e)); }
    logToFile('Parsed Leagues', leagues);
    return leagues;
  }

  async getLeague(leagueKey: string) { return this.makeRequest(`/league/${leagueKey}`); }
  ...
  async getTeam(teamKey: string) {
    ...
    let team: any = null;
    try {
      const teamData = (data as any)?.fantasy_content?.team?.[0];
      if (teamData) {
        team = {
          ...
          managers: teamData.managers?.[0]?.manager?.map((m: any) => ({
    ...
    } catch (e) { logToFile('Error parsing team', String(e)); }
    logToFile('Parsed Team', team);
    return team;
  }

  async getRoster(teamKey: string, opts?: { debugPlayerKey?: string }) { return this.getRosterByDate(teamKey, undefined, opts); }

  async getRosterWithStats(teamKey: string) { return this.makeRequest(`/team/${teamKey}/roster/players/stats`); }
  ...
  private parseRosterFromData(data: any, opts?: { debugPlayerKey?: string }): any[] {
    const roster: any[] = [];
    try {
      ...
      for (const player of playerArray) {
        ...
        if (player.player_stats?.[0]?.stats?.[0]?.stat) {
          ...
          if (opts?.debugPlayerKey && model.key === opts.debugPlayerKey) {
            logToFile('DEBUG Player Decoded Stats', { player_key: model.key, name: model.name, decodedStats, keyStats });
          }
        }
    ...
    } catch (e) { logToFile('Error parsing roster data', String(e)); }
    return roster;
  }
  ...
  async getPlayerStats(playerKey: string) {
    ...
    try {
      const player = (data as any)?.fantasy_content?.player?.[0];
      if (player?.player_stats?.[0]?.stats?.[0]?.stat) {
        ...
        for (const stat of statsArray) {
          const id = stat.stat_id?.[0]; const val = stat.value?.[0]; parsed[`stat_${id}`] = val;
        }
        playerStats = parsed;
      }
    } catch (e) { logToFile('Error parsing player stats', String(e)); }
  ...
  async getStandings(leagueKey: string) {
    ...
    const standings: any[] = [];
    try {
      const league = (data as any)?.fantasy_content?.league?.[0];
    ...
    } catch (e) { logToFile('Error parsing standings data', String(e)); }
  ...
  async getMatchups(teamKey: string) {
    ...
    const matchups: any[] = [];
    try {
      const team = (data as any)?.fantasy_content?.team?.[0];
      ...
      for (const matchup of matchupArray) {
        const teams = matchup.teams?.[0]?.team; const teamsArray = Array.isArray(teams) ? teams : teams ? [teams] : [];
        const statWinners: any[] = [];
        if (matchup.stat_winners?.[0]?.stat_winner) {
          const winners = matchup.stat_winners[0].stat_winner; const winnersArray = Array.isArray(winners) ? winners : [winners];
          for (const winner of winnersArray) { statWinners.push({ stat_id: winner.stat_id?.[0], winner_team_key: winner.winner_team_key?.[0], is_tied: winner.is_tied?.[0] === '1' }); }
        }
        matchups.push({
          ...
          teams: teamsArray.map((t: any) => ({ team_key: t.team_key?.[0], team_id: t.team_id?.[0], name: t.name?.[0], team_points: t.team_points?.[0]?.total?.[0], team_projected_points: t.team_projected_points?.[0]?.total?.[0] }))
        });
      }
    } catch (e) { logToFile('Error parsing matchups data', String(e)); }
  ...
  async getCurrentMatchup(teamKey: string, week?: string) {
    ...
    try {
      ...
      if (matchup) {
        const teams = matchup.teams?.[0]?.team; const teamsArray = Array.isArray(teams) ? teams : teams ? [teams] : [];
        const teamsWithStats = teamsArray.map((t: any) => ({
          ...
          team_stats: t.team_stats?.[0]?.stats?.[0]?.stat?.map((s: any) => ({ stat_id: s.stat_id?.[0], value: s.value?.[0] })) || []
        }));
    ...
    } catch (e) { logToFile('Error parsing current matchup data', String(e)); }
  ...
  async testFantasyPointsEndpoints(teamKey: string) {
    ...
    for (const endpoint of endpoints) {
      try { logToFile(`TESTING ENDPOINT: ${endpoint}`, 'Starting test...'); const data = await this.makeRequest(endpoint); logToFile(`SUCCESS ENDPOINT: ${endpoint}`, data); }
      catch (error: any) { logToFile(`FAILED ENDPOINT: ${endpoint}`, { error: error.message }); }
    }
  }
}
