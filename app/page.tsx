...
export default function Home() {
  ...
  const shiftDate = (deltaDays: number) => {
    // Parse date as UTC-like to avoid local DST surprises
    ...
    setDate(next);
    // auto-refresh if a team is already selected
    if (selectedTeam) {
      // ensure dateMode is 'date' so query gets applied
      setDateMode('date');
  ...
  const PopupHeader = () => (
    <div className="border-b border-zinc-700 pb-3">
      ...
      {/* Coverage controls moved to the top */}
      {step !== 'signin' && (
        <div className="mt-3 flex flex-wrap items-center gap-3">
            ...
            <div className="flex items-center gap-3">
              <div className="flex flex-col gap-1">
                <button
                  ...
                  aria-label="Next day"
                  onClick={() => shiftDate(1)}
                  className="px-2 py-1 rounded-md bg-zinc-800 hover:bg-zinc-700 border border-zinc-700"
                  title="Next day"
                >
                  ↑
                </button>
              ...
              <input
                type="date"
                className="px-3 py-1.5 rounded-md bg-zinc-900 border border-zinc-700 text-zinc-100"
                value={date}
                onChange={(e) => {
          ...
          <button
            ...
            className="ml-auto px-3 py-1.5 rounded-md bg-violet-600 hover:bg-violet-500 text-white disabled:opacity-50"
            disabled={!selectedTeam || (dateMode==='date' && !date)}
          >
            Apply
          </button>
        </div>
          ...
          {step === 'team' && (
            <div className="space-y-4">
              ...
              <div className="grid gap-4">
                ...
                {/* Coverage controls moved to header */}
              </div>
            </div>
          )}
        </div>
